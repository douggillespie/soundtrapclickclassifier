% classify all clusters in all databases.
sets = 0:14
sets = 11:14
for deployment = 3
    minCorrelation = 0.9;
...
xtraColTypes = {'double','double'}
    ...
    for i = 1:numel(sets)
        dbName = morlaisfolders(deployment, sets(i));
        [oldclusterSpecies, clusters, classParams] = classifyclusters(deployment, sets(i), 10, 1, minCorrelation);
        ...
        for c = 1:numel(clusters)
            ...
            if mod(evId,100) == 0
                fprintf('writing depl %d cluster %d of %d\n', sets(i), evId, nRow);
            end
            evData{1} = evId;
            evData{2} = evId;
            ...
            % etbl = cell2table(evData, 'variablenames', evColumns);
            % sqlwrite(con, eventTable, etbl);
            % now do all the clicks for that event.
        ...
        % clusterTimes = [clusters.start];
        % % for s = 1:2
        % porpoiseTimes = clusterTimes(find(clusterSpecies ==2));
        % stationCount(i) = numel(porpoiseTimes);
        % datehist = histc(porpoiseTimes, dBins);
        % allHist(i,:) = datehist;
        % figure(3)
        % stairs(dBins, datehist, 'LineWidth', 2)
        % hold on
        % pause(.1);
        % lgnd{i} = sprintf('Station %d', sets(i));
    end
end
% datetick
% set(gca, 'xlim', minmax(dBins));
% legend(lgnd)
%
% figure(4)
% bar(dBins, allHist(1:end,:), 'stacked')
% set(gca, 'xlim', [d1 d2]);
% datetick
% legend(lgnd)
%
% figure(5);
% bar(sets, stationCount);
% set(gca, 'xlabel', 'Station')
% set(gca, 'ylim', 'porpoise clusters / day')
